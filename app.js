...
filtered.map(x => "<article class="card"> <div class="pic"> ${ x.image_url ?"<img src="${x.image_url}" alt="${x.name}" style="width:100%;height:100%;object-fit:cover;">":"<span>${x.icon || "🛍️"}</span>"} </div> <h3>${x.name}</h3> <div class="seller">${x.seller}</div> <div class="price">R${x.price.toFixed(2)}</div> <button class="add" onclick="add(${x.id})">Add to cart</button> </article>").join("") || "<p>No products found.</p>";
}
